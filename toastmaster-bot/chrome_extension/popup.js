...
async function checkMeetingPage() {
  ...
  const isMeetingPage = meetingUrls.some(
    (url) => tab.url && tab.url.includes(url),
  );
...
function updateSpeakersList() {
  ...
  if (Object.keys(speakersList).length === 0) {
    listEl.innerHTML =
      '<div style="color:#aaa;font-size:13px">No speakers yet</div>';
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (message.action === "speakerEvaluated") {
    if (speakersList[message.speakerName]) {
      ...
      speakersList[message.speakerName].duration = message.duration || 0;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("start-btn").addEventListener("click", startMeeting);

  document
    .getElementById("next-speaker-btn")
    .addEventListener("click", setNextSpeaker);

  document
    .getElementById("end-speaker-btn")
    .addEventListener("click", endCurrentSpeaker);

  document.getElementById("stop-btn").addEventListener("click", stopMeeting);

  document
    .getElementById("speaker-name-input")
